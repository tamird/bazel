...
public class ResourceManager implements ResourceEstimator {
  ...
  private static void checkResourceAvailable(
      double available, double requested, String resourceName)
  ...
  synchronized boolean areResourcesAvailable(ResourceSet resources) {
    ...
    for (Map.Entry<String, Double> resource : resources.getResources().entrySet()) {
      ...
      if (key.equals(ResourceSet.CPU) && cpuLoadScheduling) {
        ...
        effectiveUsage = windowEstimation + currentUsage;
      }
      if (!isAvailable(available, used, effectiveUsage, requested)) {
        return false;
      }
    }
    return true;
