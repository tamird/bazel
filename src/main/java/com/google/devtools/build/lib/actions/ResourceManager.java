...
public class ResourceManager implements ResourceEstimator {
  ...
  private boolean isAvailable(
    ...
    // 2) If tracked resource usage is zero and the flag
    ...
    // 3) If effective usage plus the request is within the available resources.
    return requested == 0
  ...
  synchronized boolean isCpuAvailable(Map.Entry<String, Double> resource) {
    ...
    if (cpuLoadScheduling) {
      ...
      // Allow the first action with no CPU capacity; otherwise cap running actions at x3 CPU.
